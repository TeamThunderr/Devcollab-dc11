...
export function ViewerOverview({ projectId: propsId }: ViewerOverviewProps = {}) {
  ...
  const projectActivities = React.useMemo(() => {
    return activities
      .filter((a: any) => !projectId || String(a.projectId) === String(projectId))
  ...
  const projectSprints = React.useMemo(() => {
    ...
    return sprints.filter((s: any) => String(s.projectId) === String(projectId));
  }, [sprints, projectId]);
  ...
  const reviewTasks = projectTasks.filter(t => {
    const s = toFrontendStatus(t.status);
    return s === "In Review" || t.status === "IN_REVIEW" || t.status === "Review";
  });
  const todoTasks = projectTasks.filter(t => {
    const s = toFrontendStatus(t.status);
  ...
  const urgentTasks = projectTasks.filter(t => t.priority === "P0" || t.priority === "Urgent");
  const highTasks = projectTasks.filter(t => t.priority === "P1" || t.priority === "High");
  const normalTasks = projectTasks.filter(t => t.priority === "P2" || t.priority === "Medium" || !t.priority || t.priority === "Normal");
  const lowTasks = projectTasks.filter(t => t.priority === "P3" || t.priority === "Low");
  ...
  return (
    <div className="space-y-6 pb-12 w-full max-w-7xl mx-auto pt-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
        ...
        <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-[#2C2C2C] rounded-xl p-4 shadow-sm flex flex-col justify-between space-y-2.5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Sprints</span>
            <Flag className="w-4 h-4 text-gray-600 dark:text-gray-400" />
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-gray-900 dark:text-gray-100 font-mono">{projectSprints.length}</span>
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded bg-gray-100 dark:bg-[#191919] text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-[#2C2C2C]">Active</span>
          </div>
          <span className="text-[11px] text-gray-500 dark:text-gray-400">Target: {formatDate((project as any)?.endDate || project?.createdAt)}</span>
        </div>
      </div>

      {/* 3. ROW 1: Consolidated Specifications & Priority Breakdown with Live Backlog Deliverables */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-stretch w-full">
        {/* Card 1 (Left): Project Specifications & Backlog Status (100% Live Data Only) */}
              ...
              <div className="space-y-2 text-sm">
                ...
                {projectTasks.length === 0 && (
                  <div className="p-6 text-center text-xs text-gray-500 border border-dashed border-gray-200 dark:border-[#2C2C2C] rounded-lg">
                    No tasks across your project yet.
                  </div>
                )}
              </div>
            </div>
      ...
      {/* 4. ROW 2: Roadmap Sprints & Execution Timeline + Recent Activity Feed (Strictly Live Sprints & Activities Only!) */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-stretch w-full">
            ...
            {projectSprints && projectSprints.length > 0 ? (
              <div className="space-y-3.5">
                {projectSprints.slice(0, 4).map((s: any) => {
                  const sTasks = projectTasks.filter(t => String((t as any).sprintId) === String(s.id));
                    ...
                    <div key={act.id} className="relative group">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="text-xs md:text-sm text-gray-600 dark:text-gray-300 leading-snug">
                          ...
                          {act.action || act.actionType || "updated state"}
                        </div>
